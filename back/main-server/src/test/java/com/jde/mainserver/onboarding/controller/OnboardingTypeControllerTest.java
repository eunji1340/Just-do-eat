...
class OnboardingTypeControllerTest {
	...
	void typeResult_success() throws Exception {
		...
		mockMvc.perform(get("/onboarding/result/types/MPST"))
				...
				.andExpect(jsonPath("$.data.goodMatch[0].imagePath").value("http://localhost/api/mbtis/NPSD.png"))
				.andExpect(jsonPath("$.data.badMatch[0].type").value("MQAD"))
				.andExpect(jsonPath("$.data.badMatch[0].imagePath").value("http://localhost/api/mbtis/MQAD.png"))
				.andExpect(jsonPath("$.data.imagePath").value("http://localhost/api/mbtis/MPST.png"));
	}
